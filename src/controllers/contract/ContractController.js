...
export const getIpfsUrl = (blockchain, getCloudGatewayUrl) =>
  ((blockchain === 'solana' ||
...
const getContractType = (blockchain) => {
  if (
    ...
    return 'ethereum';
  } else if (blockchain == 'solana' || blockchain == 'solanadevnet') {
    return 'solana';
  }
};
...
export class ContractController {
  ...
  async deployContract(deployerAddress, name, symbol, totalSupply, onError) {
    ...
    const compiledProxy =
      ((blockchain === 'ethereum' || blockchain === 'polygon') &&
        ProxyERC721a) ||
      ((blockchain === 'rinkeby' || blockchain === 'mumbai') &&
        ProxyERC721aTestnet) ||
      null;
  ...
  async mint(walletAddress, count, whitelist) {
    ...
    if (type === 'solana') {
      await mintV2(
        (blockchain === 'solanadevnet' && 'devnet') || 'mainnet',
        contractAddress,
        walletAddress,
      );
      return;
    ...
    if (type === 'ethereum') {
      ...
      if (isPresaleOpen) {
        if (!whitelist || !whitelist.length) {
          throw new Error(
            'Whitelist not available! Pre Sale miniting not possible.',
          );
        }
